import customtkinter as tki, os, sys, subprocess as s, pyshortcuts as pys
...
try: os.chdir('src/installer'); print('Dir changed to src/installer')
except: print('Could not change dir to src/installer')

# installPath = %appdata%/SussyLauncher/

def install(): 
    with open('scripts/installScript.bat','r') as file: content = file.readlines()
    ...
    with open('scripts/installScript_Customised.bat','w') as file: file.writelines(newContent)
    ...
    with open(f'{a}run.cmd','a') as file:
        ...
        file.write('start "" "SussyLauncher/SussyLauncher V1.5/SussyLauncher V1.5.exe"')
        
    os.system(f'mkdir "{a}SussyLauncher\\SussyLauncher V1.5\\minecraft_launcher_lib"')
    open(f'{a}SussyLauncher\\SussyLauncher V1.5\\minecraft_launcher_lib\\version.txt','w').close()
    
    pys.make_shortcut(script=f'{a}run.cmd',name='Sussy Launcher V1.5',desktop=True,startmenu=True,terminal=False)
...
title = tklabel(master=main,text=titleText,font=tkfont(size=25),anchor='center',height=50,width=100)
title.grid(row=0,column=0)

text = tklabel(master=main,text=text,font=tkfont(size=20),anchor='center',height=50,width=500,wraplength=500)
text.grid(row=1,column=0)

input = tktextbox(master=main,corner_radius=10,width=350,height=25,activate_scrollbars=False)
...
checkbox = tkcheckbox(master=main,width=50,height=50,text='Custom path',command=custom_path,variable=checkbox_state,onvalue='on',offvalue='off')
checkbox.grid(row=3,column=0)


installButton = tkbutton(master=main,text='Install',font=tkfont(size=30),command=install)
installButton.grid(row=4,column=0)

filler = tklabel(master=main,width=1000,height=1000,text='Ur not supposed to see this',anchor='center')
